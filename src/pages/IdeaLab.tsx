      ...
      <div className="grid grid-cols-12 gap-6">
        <div className="col-span-8">
          <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
            ...
            <div className="p-6">
              {activeTab === 'brainstorm' && (
                <div className="space-y-4">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 disabled:bg-slate-600 text-slate-900 font-medium rounded-lg transition-colors"
                  >
              ...
              {activeTab === 'fusion' && (
                <div className="space-y-4">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 disabled:bg-slate-600 text-slate-900 font-medium rounded-lg transition-colors"
                  >
                    {isLoading ? <Loader2 size={18} className="animate-spin" /> : <GitMerge size={18} />}
                    {isLoading ? '分析中...' : '分析并融合'}
                  </button>
                </div>
              )}
              ...
              {activeTab === 'extract' && (
                <div className="space-y-4">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-amber-500 hover:bg-amber-600 disabled:bg-slate-600 text-slate-900 font-medium rounded-lg transition-colors"
                  >
                    {isLoading ? <Loader2 size={18} className="animate-spin" /> : <FileSearch size={18} />}
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    {genreGuides.map((genre) => (
                      <div
                        key={genre.name}
                        className="bg-slate-700/30 border border-slate-600 rounded-xl p-4 hover:border-amber-500/50 transition-colors cursor-pointer"
                      >
                        <h3 className="font-semibold text-white mb-2">{genre.name}</h3>
                        <p className="text-sm text-slate-400 mb-3">{genre.description}</p>
                        <div className="space-y-2">
                          <div>
                            <span className="text-xs text-amber-400">必备桥段:</span>
                            <div className="flex flex-wrap gap-1 mt-1">
                              {genre.beats.map((beat) => (
                                <span key={beat} className="text-xs px-2 py-0.5 bg-amber-500/20 text-amber-400 rounded-full">
                                  {beat}
                                </span>
                              ))}
                            </div>
              ...
              {result && (
                <div className="mt-6 p-4 bg-slate-700/30 border border-slate-600 rounded-xl">
                  <h3 className="font-semibold text-white mb-2">生成结果</h3>
        ...
        <div className="col-span-4">
          <div className="bg-slate-800 rounded-xl border border-slate-700 p-6">
            <h3 className="font-semibold text-white mb-4">灵感小贴士</h3>
            <div className="space-y-4">
              <div className="bg-slate-700/30 rounded-lg p-4">
                <p className="text-sm text-slate-300">
                  <span className="text-amber-400 font-medium">✦</span> 尝试用"如果..."开头的问题来激发创意
                </p>
              </div>
              <div className="bg-slate-700/30 rounded-lg p-4">
                <p className="text-sm text-slate-300">
                  <span className="text-amber-400 font-medium">✦</span> 融合两种截然不同的作品往往能产生惊喜
                </p>
              </div>
              <div className="bg-slate-700/30 rounded-lg p-4">
                <p className="text-sm text-slate-300">
                  <span className="text-amber-400 font-medium">✦</span> 梦境和日常观察是最好的创意来源
                </p>
              </div>
              <div className="bg-slate-700/30 rounded-lg p-4">
                <p className="text-sm text-slate-300">
                  <span className="text-amber-400 font-medium">✦</span> 不要害怕荒诞的想法，伟大的故事往往始于疯狂
                </p>
              </div>
            </div>
          </div>
